...
Intercept: when all of our predictors are equal to zero
...
new_data = tibble(treatment = c("Neighbors", "Control"),
                  solo = TRUE,
                  primary_04 = "Yes")

posterior_predict(fit_1, newdata = new_data) %>%
  as_tibble() %>%
  mutate(diff = `1` - `2`) %>%
  ggplot(aes(x = diff)) +
...
posterior_epred(fit_1, newdata = new_data) %>%
  as_tibble() %>%
  mutate(diff = `1` - `2`) %>%
  ggplot(aes(x = diff)) +
  geom_histogram(aes(y = after_stat(count / sum(count))), bins = 100)
